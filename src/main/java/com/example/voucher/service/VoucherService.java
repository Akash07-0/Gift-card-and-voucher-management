...
public class VoucherService {
    ...
    public RedemptionResponse redeem(
        ...
        voucher.setCurrentUsage(
            voucher.getCurrentUsage() + 1
        );

        voucherRepository.save(voucher);

        return RedemptionResponse.from(
            redemptionRepository.save(redemption)
        );
    }

    public List<RedemptionResponse> myHistory(
        String email
    ) {
        ...
        return redemptionRepository
            .findByUserIdOrderByRedeemedAtDesc(user.getId())
            .stream()
